...
func (m *Map[T]) Load(key any) (value T, ok bool) {
	...
	return e.load(m.getExpunged())
}

func (e *entry[T]) load(expunged *T) (value T, ok bool) {
	p := e.p.Load()
	if p == nil || p == expunged {
		var v T
		return v, false
...
func (e *entry[T]) unexpungeLocked(expunged *T) (wasExpunged bool) {
...
func (m *Map[T]) LoadOrStore(key any, value T) (actual T, loaded bool) {
	...
	if e, ok := read.m[key]; ok {
		actual, loaded, ok := e.tryLoadOrStore(value, m.getExpunged())
		if ok {
	...
	read = m.loadReadOnly()
	if e, ok := read.m[key]; ok {
		if e.unexpungeLocked(m.getExpunged()) {
			m.dirty[key] = e
		}
		actual, loaded, _ = e.tryLoadOrStore(value, m.getExpunged())
...
func (e *entry[T]) tryLoadOrStore(i T, expunged *T) (actual T, loaded, ok bool) {
	p := e.p.Load()
	if p == expunged {
		var v T
...
func (e *entry[T]) delete(expunged *T) (value T, ok bool) {
	for {
		p := e.p.Load()
		if p == nil || p == expunged {
			return value, false
...
// unchanged.
func (e *entry[T]) trySwap(i *T, expunged *T) (*T, bool) {
	for {
		p := e.p.Load()
		if p == expunged {
			return nil, false
...
func (m *Map[T]) CompareAndSwap(key any, old, new T) bool {
	...
	swapped := false
	if e, ok := read.m[key]; ok {
		swapped = e.tryCompareAndSwap(old, new, m.getExpunged())
	} else if e, ok := m.dirty[key]; ok {
		swapped = e.tryCompareAndSwap(old, new, m.getExpunged())
...
func (e *entry[T]) tryExpungeLocked(expunged *T) (isExpunged bool) {
	p := e.p.Load()
	for p == nil {
